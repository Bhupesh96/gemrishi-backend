...
export const protectAdmin = async (req, res, next) => {
  ...
  if (!token) {
    return res.status(401)
      ...
      }).json({ success: false, msg: "Unauthorized access: No token provided, Try Login Again" });
  }
  ...
  try {
    ...
    if (error instanceof jwt.TokenExpiredError) {
      return res.status(401)
        ...
        secure: process.env.NODE_ENV === "production",
        sameSite: process.env.NODE_ENV === "production" ? "None" : "Lax",
        }).json({ success: false, msg: "Unauthorized access: Your session token has expired, Try Login Again" });
    }
    return res.status(401)
      .clearCookie("token", {
      httpOnly: true,
      secure: process.env.NODE_ENV === "production",
      sameSite: process.env.NODE_ENV === "production" ? "None" : "Lax",
      }).json({ success: false, msg: "Unauthorized access: Invalid session token, Try Login Again" });
  }
...
export const protect = asyncHandler(async (req, res, next) => {
  const token = req.header("Authorization")?.split(" ")[1] || req.cookies.usertoken;
  if (!token) {
    return res.status(401)
  ...
  const user = await User.findById(decoded.id).select('email fullName mobileNo');
  if (!user) {
    return res.status(404).json({ success: false, msg: "No user found for this token" });
  }
...
export const protectRetailer = asyncHandler(async (req, res, next) => {
  ...
  const user = await Retailer.findById(decoded.id).select('email fullName mobileNo isActive isBlocked');
  if (!user) {
    return res.status(404).json({ success: false, msg: "No Retailer found" });
  }

  if (user.isActive === false) {
    return res.status(401).json({ success: false, msg: "Your account has been Deactivated, Please Reactivate Your Account." });
  }

  if (user.isBlocked === true) {
    return res.status(401).json({ success: false, msg: "Your account has been Blocked By Wholesaler, Please Contact Your Wholesaler." });
  }
...
export const checkUserLoggedIn = async (req, res, next) => {
  try {
    ...
    if (token) {
      ...
      // If token exists, run protect middleware
